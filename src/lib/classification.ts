// Event Classification System for Founder Bleed
...
export type Vertical = 'engineering' | 'business';
...
export function classifyEvent(
  ...
  const vertical: Vertical = ENGINEERING_AREAS.includes(businessArea)
    ? 'engineering'
    : 'business';
  ...
  // Solo founder adjustment - CRITICAL INVARIANT
  // If solo founder, "founder" tier becomes "unique"
  ...
  return {
    ...
    vertical,
    confidence,
    keywordsMatched: [...new Set(matchedKeywords)], // deduplicate
  };
}

// Check if user is a solo founder
export function isSoloFounder(
  teamComposition: Record<string, number> | null
): boolean {
  if (!teamComposition) return true; // Default to solo founder if not set

  const founderCount = teamComposition.founder ?? 1;
  const otherTeamMembers = Object.entries(teamComposition)
    .filter(([key]) => key !== 'founder')
    .reduce((sum, [, count]) => sum + (count || 0), 0);

  return founderCount === 1 && otherTeamMembers === 0;
}
...
export function getAvailableTiers(
  ...
  if (isSoloFounder(teamComposition)) {
    // Solo founders only see 4 tiers
    return ['unique', 'senior', 'junior', 'ea'];
  }
  return ['unique', 'founder', 'senior', 'junior', 'ea'];
}
